...
static inline struct reptar_sp6_led *pdev_to_sp6_led(
		struct platform_device *dev) {
	return platform_get_drvdata(dev);
}

static inline struct reptar_sp6_led *leddev_to_sp6_led(
		struct led_classdev *led_cdev) {
return container_of(led_cdev, struct reptar_sp6_led, cdev);
}

static void reptar_sp6_led_set(struct led_classdev *led_cdev,
...
/* ... */
led->reg = ioremap(fpga_resource->start + pdata->reg_offset, sizeof(uint16_t));
...
if (ret) {
	dev_err(&pdev->dev, "Unable to register led device, error: %d\n", ret);
	return ret;
}
...
static struct platform_driver reptar_sp6_led_driver = { .probe =
