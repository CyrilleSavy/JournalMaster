...
static uint64_t sp6_read(void *opaque, hwaddr addr, unsigned size)
	...
	break;
    case SP6_7SEG2:
	reptar_sp6_7segs_read(SP6_7SEG2);
	break;
    case SP6_7SEG3:
	reptar_sp6_7segs_read(SP6_7SEG3);
	break;
...
static const MemoryRegionOps sp6_ops =
    {
    .read = sp6_read, .write = sp6_write, .endianness = DEVICE_NATIVE_ENDIAN,
    };
...
static const TypeInfo reptar_sp6_info =
    {
    .name = "reptar_sp6", .parent = TYPE_SYS_BUS_DEVICE, .instance_size =
	    sizeof(sp6_state_t), .instance_init = sp6_init, .class_init =
	    sp6_class_init,
    };
