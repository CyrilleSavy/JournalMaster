...
static void reptar_init(MachineState *machine)
    ...
    s->cpu = omap3_mpu_init(sysmem, omap3430, ram_size, NULL, NULL,
	    serial_hds[0], NULL);
	...
	NULL, "reptar.nand_flash", pflash_size, blk_by_legacy_dinfo(pflash),
		sector_len, pflash_size / sector_len, 1, 2, 0, 0, 0, 0, 0x555,
		0x2AA, 0))
	    {
    ...
    /* Spartan6 FPGA mappée al'adresse 0x1800_0000, sans IRQ*/
    s->sp6 = sysbus_create_simple("reptar_sp6", 0x18000000, NULL);

    sysbus_connect_irq(SYS_BUS_DEVICE(s->sp6), 0,
	    qdev_get_gpio_in(s->cpu->gpio, 10));
    }

static QEMUMachine reptar_machine =
    {
    .name = "reptar", .desc = "REPTAR board (based on OMAP3530)", .init =
	    reptar_init,
    };
